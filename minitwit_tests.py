...
class MiniTwitTestCase(unittest.TestCase):
    ...
    def add_message(self, text):
        ...
        if text:
            assert "Your message was recorded" in str(rv.data)
        return rv
    ...
    def test_simple(self):
        ...
        assert 'You were successfully registered ' \
               'and can login now' in str(rv.data)
    ...
    def test_register(self):
        ...
        assert 'You were successfully registered ' \
               'and can login now' in str(rv.data)
        rv = self.register('user1', 'default')
        assert 'The username is already taken' in str(rv.data)
        rv = self.register('', 'default')
        assert 'You have to enter a username' in str(rv.data)
        rv = self.register('meh', '')
        assert 'You have to enter a password' in str(rv.data)
        rv = self.register('meh', 'x', 'y')
        assert 'The two passwords do not match' in str(rv.data)
        rv = self.register('meh', 'foo', email='broken')
        assert 'You have to enter a valid email address' in str(rv.data)
    ...
    def test_message_recording(self):
        ...
        rv = self.app.get('/')
        assert 'test message 1' in str(rv.data)
        assert '&lt;test message 2&gt;' in str(rv.data)
    ...
    def test_timelines(self):
        ...
        assert 'the message by foo' in str(rv.data)
        assert 'the message by bar' in str(rv.data)

        # bar's timeline should just show bar's message
        rv = self.app.get('/')
        assert 'the message by foo' not in str(rv.data)
        assert 'the message by bar' in str(rv.data)

        # now let's follow foo
        rv = self.app.get('/foo/follow', follow_redirects=True)
        assert 'You are now following &#34;foo&#34;' in str(rv.data)
        ...
        rv = self.app.get('/foo')
        assert 'the message by foo' in str(rv.data)
        assert 'the message by bar' not in str(rv.data)

        # now unfollow and check if that worked
        rv = self.app.get('/foo/unfollow', follow_redirects=True)
        assert 'You are no longer following &#34;foo&#34;' in str(rv.data)
        rv = self.app.get('/')
        assert 'the message by foo' not in str(rv.data)
        assert 'the message by bar' in str(rv.data)
